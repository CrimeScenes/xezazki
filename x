...
UserInputService.InputBegan:Connect(function(input, isProcessed)
    if not isProcessed and input.UserInputType == Enum.UserInputType.MouseButton1 then
        ...
        if closestPlayer then
            ...
            local remoteEvent = Replicated_Storage:FindFirstChild("MainEvent")
            local Argument = gameSettings.Argument

            if remoteEvent and Argument then
                local success, err = pcall(function()
                    remoteEvent:FireServer(Argument, mousePosition)
                end)
                if not success then
                    -- Optional error handling: silently handle if FireServer fails
                end
            end
        end
    end
...
if remoteEvent then
    hookfunction(remoteEvent.FireServer, function(self, ...)
        ...
        if args[1] == gameSettings.Argument then
